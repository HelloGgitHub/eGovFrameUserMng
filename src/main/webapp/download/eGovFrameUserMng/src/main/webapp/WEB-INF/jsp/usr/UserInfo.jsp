...
function required() {
	if($.trim($("#inUserId").val()).length == 0){
		...
		alert("비밀번호는 필수 입력값입니다.");$("#inPassword").focus();return false;
	} else if($.trim($("#inPasswordHint").val()).length == 0){
		alert("비밀번호 힌트는 필수 입력값입니다.");$("#inPasswordHint").focus();return false;
	} else if($.trim($("#inPasswordCnsr").val()).length == 0){
		alert("비밀번호 정답은 필수 입력값입니다.");$("#inPasswordCnsr").focus();return false;
	}
}
   //입력값 길이 체크
function maxlength() { 
	if($.trim($("#inUserId").val()).length >= 20){
		...
		alert("비밀번호 정답은 100자 이상 입력할수 없습니다.");$("#inPassword").focus();return false;
	} else if($.trim($("#inAreaNo").val()).length >= 4){
		alert("집지역번호는 4자 이상 입력할수 없습니다.");$("#inAreaNo").focus();return false;
	} else if($.trim($("#inMiddleTelno").val()).length >= 5){
		alert("집중간전화번호는 4자 이상 입력할수 없습니다.");$("#inMiddleTelno").focus();return false;
	} else if($.trim($("#inEndTelno").val()).length >= 5){
		alert("집마지막전화번호는 4자 이상 입력할수 없습니다.");$("#inEndTelno").focus();return false;
	} else if($.trim($("#inUserFxnum").val()).length >= 15){
		alert("팩스번호는 15자 이상 입력할수 없습니다.");$("#inUserFxnum").focus();return false;
	} else if($.trim($("#inMoblphonNo").val()).length >= 15){
		alert("핸드폰번호는 15자 이상 입력할수 없습니다.");$("#inMoblphonNo").focus();return false;
	} else if($.trim($("#inUserEmailAdres").val()).length >= 50){
		alert("이메일주소는 50자 이상 입력할수 없습니다.");$("#inUserEmailAdres").focus();return false;
	} else if($.trim($("#inAdres").val()).length >= 100){
		alert("주소는 100자 이상 입력할수 없습니다.");$("#inAdres").focus();return false;
	}
}
...
function ckEmail() { 
	...
	return CheckEmail(  $.trim($('#inUserEmailAdres').val())  );
} 
...
function fn_UserCk(){
	if($.trim($("#inUserId").val()).length == 0){
		alert("사용자 아이디는 필수 입력값 입니다.");$("#inUserId").focus();return;
	}
	var rtnData  = new Object();
	...
	<table class="wTable" summary="사용자 정보를 출력합니다.">
		...
		<tbody>
			...
			<tr>
				...
				<td class="left">
					<input id="inPassword" name="inPassword"  autocomplete="off" title="비밀번호 입력" onchange="inputModify()" type="password" value="" size="50" maxlength="20"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="left">
					<input name="inPassword2" id="inPassword2"  autocomplete="off" title="비밀번호확인 입력" type="password" size="50" maxlength="20" />
				</td>
			</tr>
			...
			<!-- 성별구분코드 -->
			<tr>
				<th><label for="inSexdstnCode">성별구분</label></th>
				<td class="left">
					<select id="inSexdstnCode" name="inSexdstnCode" title="성별구분 선택">
						<option value="" selected="selected">--선택하세요--</option>
			...
			<tr>
				...
				<td class="left">
	                    <input id="inUserFxnum" name="inUserFxnum"  autocomplete="off" class="txaIpUmt" title="팩스번호 입력" type="text" value="" size="20" maxlength="15"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="left">
	                    <input id="inMoblphonNo" name="inMoblphonNo" autocomplete="off" class="txaIpUmt" title="핸드폰번호 입력" type="text" value="" size="20" maxlength="15"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="left">
	                    <input id="inUserEmailAdres" name="inUserEmailAdres" autocomplete="off" class="txaIpUmt" title="이메일주소 입력" type="text" value="" size="30" maxlength="50"/>
				</td>
			</tr>
			...
			<tr>
				<th><label for="inUserSttus">사용자가입상태</label></th>
				<td class="left">
