...
function required() {
	if($.trim($("#inGroupId").val()).length == 0){
		alert("그룹 아이디는 필수 입력값입니다.");$("#inGroupId").focus();return false;
	}
}
...
function maxlength() {
	if($.trim($("#inGroupId").val()).length >= 20){
		alert("그룹 아이디는 20자 이상 입력할수 없습니다.");$("#inGroupId").focus();return false;
	} else if($.trim($("#inGroupNm").val()).length >= 50){
		alert("그룹명은 50자 이상 입력할수 없습니다.");$("#inGroupNm").focus();return false;
	} else if($.trim($("#inGroupDc").val()).length >= 100){
		alert("그룹설명은 100자 이상 입력할수 없습니다.");$("#inGroupDc").focus();return false;
	}
}
...
function fn_UserList(){
	location.href="/UserGroupSet?callType=r&groupId="+$("#inGroupId").val();
}
